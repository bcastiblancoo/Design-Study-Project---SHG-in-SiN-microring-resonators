...
data = emc.get(variable='sweep_data', simulation_name='dispersionSiN_SH')

vals = np.array(data['values'])           
neff = np.array(data['effective_index'])  
...
print("λ(min), λ(max) SH =", wav_nm_raw.min(), wav_nm_raw.max(), "nm")


df = pd.DataFrame({'lambda_nm': wav_nm_raw, 'neff': neff_raw})
df.to_csv('SH_modes_raw.csv', index=False)
print("Saved SH_modes_raw.csv with", len(df), "points.")

-
n_eff_spl = UnivariateSpline(wav_nm_raw, neff_raw, s=0, k=4)
n_eff_spl_2d = n_eff_spl.derivative(n=2)

lam_min = wav_nm_raw.min()
lam_max = wav_nm_raw.max()
...
xticks_nm = np.linspace(lam_min, lam_max, 5)
...
fig.savefig('dispersionSiN_SH.png', dpi=600, bbox_inches='tight')
print("Saved dispersionSiN_SH.png")
